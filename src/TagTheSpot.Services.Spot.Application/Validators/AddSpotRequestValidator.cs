...
{
    public sealed class AddSpotRequestValidator
        : AbstractValidator<AddSpotRequest>
    {
        public AddSpotRequestValidator()
        {
            ...
            RuleFor(x => x.SkillLevel)!
                .MustBeEnumValue<AddSpotRequest, SkillLevel>()
                .When(x => !string.IsNullOrWhiteSpace(x.SkillLevel));

            RuleFor(x => x.Accessibility)!
                .MustBeEnumValue<AddSpotRequest, Accessibility>()
                .When(x => !string.IsNullOrWhiteSpace(x.Accessibility));

            RuleFor(x => x.Condition)!
                .MustBeEnumValue<AddSpotRequest, Condition>()
